...
nums = [23, 11, 32, 30, 17]
result = sorted(nums, reverse=True)  # 对容器排序，  注意不原地修改， 要给新变量
print(nums) # 原始不变化
print(result) # 新的排列后的列表

# 1.2 若列容器中的元素：既有数字，又有字符串，那就会报错。
# nums = [23, 11, 32, 30, 17, '尚硅谷'] # 报错
sorted(nums)

# 1.3 若列容器中的元素：都是字符串，则按照字符串的 Unicode 编码大小进行排序。
msg_list = ['北京', '尚硅谷', '你好']
result = sorted(msg_list) # unicode排序
print(msg_list)
print(result)

# 2.使用内置的 len 函数，获取容器中元素的总数量，返回值是：元素总数量。
nums = [10, 20, 10, 30, 10, 40, [50, 60, 70]]
result = len(nums) # 7， 嵌套元素算一个
print(result)
...
msg_list = ['北京', '尚硅谷', '你好']
result = max(msg_list) # unicode
print(msg_list)
print(result)

# 3.4 max 函数也可以接收多个值，并筛选出最大值
result = max(33, 45, 12, 78, 99) # 没有传入容器， 用独立数字
print(result)
