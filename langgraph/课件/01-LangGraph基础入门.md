...
如下图所示(start和end是开始和结束标志，用边连接，所有的数据再状态里面)
...
</br>（超步，就是一次执行，三个步骤：绿色的已经执行了，还要执行黄色的边，
第一步： 路由，一两百个节点，复杂的执行逻辑，路由=上一个节点执行完了，下一步执行谁，需要选择判断，要在edge写清楚逻辑
第二步：执行，node_2的执行代码开始运行，作为函数，需要输入和输出，输入一般是state状态，读取当前的信息，然后开始执行逻辑，产生所有内容，暂时不更新state
第三步：要把node——2的返回结果，执行的日志，cur_id一些信息合并到state，
超步，是最小的一个执行单元，例外是start和end）
</br>（还有checkpoint检查点，执行步骤当中，把状态暂存，后面讲）
...
### 1.3.1. Graph API
（根据图的概念写，用edge连接node，构成流程图的形式来定义，  适用于：复杂分支， 比较推荐，比较复杂的多节点流程）
...
（只有最小改造，线性，简单的，就是原来的项目根本没有用langgraph，现在想要使用，想要利用持久化，中断回复的功能，为了不改变结构，用function api去对接之前的代码，方便，如果一开始用langgraph做大型项目，就用graph api）
